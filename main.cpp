...
vector<string> dijkstras(map<string,OutDegree> graph,tuple<string,OutDegree> s,tuple<string,OutDegree> e){
    PriorityQueue q;
    ...
    while (q.getSize()!=0){
        ...
        for (int i=0;i<graph.at(best).outDegree;i++){
            if (find(closed.begin(),closed.end(),bestOut[i].destination)!=closed.end()){
                continue;
            }
...
int main() {
    ...
    board.insert(make_pair<string,OutDegree>("Toronto",OutDegree(new Edge[4]{Edge("Sault St.Marie",2),Edge("Duluth",6),Edge("Montreal",3),Edge("Pittsburgh",2)},4)));
    ...
    board.insert(make_pair<string,OutDegree>("Nashville",OutDegree(new Edge[5]{Edge("Atlanta",1),Edge("Saint Louis",2),Edge("Raleigh",2),Edge("Little Rock",3),Edge("Pittsburgh",4)},5)));
    ...
    string end="Kansas City";

    vector<string> test=dijkstras(board,tuple<string,OutDegree>(start,board.at(start)),tuple<string,OutDegree>(end,board.at(end)));
